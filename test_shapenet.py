...
def get_parser():
    ...
    parser.add_argument('opts', help='config/shapenet_cfg.yaml', default=None, nargs=argparse.REMAINDER)
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    val_data = ShapeNetPart(split='test', data_root=args.data_root, transform=None)
    if args.distributed:
...
def validate(val_loader, model, criterion):
    ...
    for i, (b, lstCoordSize, coord, feat, target, offset,label) in enumerate(val_loader):
        ...
        vis_util.write_ply_color_shapenet(coord, output.cpu().numpy(), os.path.join(args.save_folder, str(label.item())+'_'+str(i) +'_pred'+'.obj'))
        vis_util.write_ply_color_shapenet(coord, target.cpu().numpy(), os.path.join(args.save_folder, str(label.item())+'_'+str(i)+'_label' + '.obj'))
        if args.multiprocessing_distributed:
            loss *= n
