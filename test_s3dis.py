...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/s3dis_cfg.yaml', help='config file')
    parser.add_argument('opts', help='see config/s3dis_cfg.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def get_logger():
    ...
    logging.basicConfig(filename='exp/s3dis/training.log')
...
def main():
    ...
    if args.arch == 'lgpa':
        from model.lgpa_s3dis import lgpa_repro as Model
    else:
...
def test(model, criterion, names):
    ...
    for idx, item in enumerate(data_list):
        ...
        if os.path.isfile(pred_save_path) and os.path.isfile(label_save_path):
            ...
            vis_util.write_ply_color_s3dis(coord, pred, os.path.join(args.save_folder, item +'_pred'+'.obj'))
            vis_util.write_ply_color_s3dis(coord, label, os.path.join(args.save_folder, item+'_label' + '.obj'))
