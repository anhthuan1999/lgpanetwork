...
def interpolation(xyz, new_xyz, feat, offset, new_offset, k=3):
    ...
    for i in range(k):
        arrUnit = idx[:, i].clone()
        if torch.max(arrUnit.long())>=feat.shape[0]:
            print(torch.max(idx[:, i]))
            print(feat.shape[0])
            print('---')
            arrUnit[arrUnit>=feat.shape[0]] = torch.max(idx[:, i])-1

        new_feat += feat[arrUnit.long(), :] * weight[:, i].unsqueeze(-1)
    return new_feat
